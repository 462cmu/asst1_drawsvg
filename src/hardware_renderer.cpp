...
void HardwareRenderer::begin2DDrawing() {
  ...
  glPushMatrix();
  glLoadIdentity();
  glOrtho( 0, context_w, context_h, 0, 0, 1 );
...
void HardwareRenderer::rasterize_image(float x0, float y0,
  ...
  // use mip level 0 and let opengl 
  // take care of its own mipmaping
  ...
  glBegin(GL_QUADS);
  glTexCoord2f(0.0, 1.0); glVertex2f(x0 + 0.5, y1 + 0.5);
  glTexCoord2f(1.0, 1.0); glVertex2f(x1 + 0.5, y1 + 0.5);
  glTexCoord2f(1.0, 0.0); glVertex2f(x1 + 0.5, y0 + 0.5);
  glTexCoord2f(0.0, 0.0); glVertex2f(x0 + 0.5, y0 + 0.5);
  glEnd();
